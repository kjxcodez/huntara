...
Contributing to HUNTARA requires adhering to our development philosophy, coding standards, branching strategies, and testing requirements to help maintain a stable, clean, and secure local-first codebase.
...
3. **No Code Duplication**: Maintain modular package boundaries. Do not duplicate logic between the Hono REST API server (`apps/api/`) and the Electron application (`apps/desktop/`). Use shared workspace packages under `packages/` (e.g., `@huntara/schema`).

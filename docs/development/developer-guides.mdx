...
HUNTARA supports modular extension of its scraper plugins, agent tools, database schemas, and IPC channels. This guide details the steps to implement and register these modules.
...
2. Implement the `BasePlugin` interface:
   ```typescript
   import { Job } from '@huntara/schema';
...
2. Implement the executor class under `packages/agent-runtime/src/tools/` implementing the `ToolExecutor` interface:
   ```typescript
   import { SendSlackSchema } from '@huntara/agent-core';
...
1. Create the repository class under `apps/desktop/src/main/database/repositories/` (e.g. `campaign-repo.ts`):
   ...
   import { Campaign } from '@huntara/schema';
...
2. Implement the listener in `apps/desktop/src/main/ipc/`:
   ...
   import { SLACK_CHANNELS } from '@huntara/schema';

...
1. Select the packages that have changed (e.g. `@huntara/desktop`).
...
This runs `electron-builder --win` under the hood. The resulting installer will be located under `apps/desktop/dist/HUNTARA-<version>-win-x64.exe`.
...
2. The application compresses the daily log files (`huntara_*.jsonl`) and configuration states into a single ZIP file.
